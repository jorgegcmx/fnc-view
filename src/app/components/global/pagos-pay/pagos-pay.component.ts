        ...
        paypal.Buttons({
            ...
            onApprove: (data: any, actions: any) => {
                return actions.order.capture().then((details: any) => {
                    ...
                    const nombre_pay = details.payer.name.given_name + ' ' + details.payer.name.surname;
                    const email_pay = details.payer.email_address;
                    const total_pay = details.purchase_units[0].amount.value;
                    this.registro(id_pay, status_pay, nombre_pay, email_pay, total_pay, "PAYPAL");
                });
            },
            onError: (err: any) => {
                console.log(err);
                alert("¡Error! al procesar tú pago, no se desconto el importe a tu cuenta, intenta con otro metodo de pago");
            }
        }).render(this.paypalElement.nativeElement);
    }

    registro(id_pay: any, status_pay: any, nombre_pay: any, email_pay: any, total_pay: any, method_pay: any) {
